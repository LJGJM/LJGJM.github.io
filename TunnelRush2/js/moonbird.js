...
function _emscripten_run_script(ptr) {
	...
	tmp = tmp.replace("PokiSDK.displayAd(this.displayContainer, this.displaySize);", "/*PokiSDK.displayAd(this.displayContainer, this.displaySize);*/");
	tmp = tmp.replace('this.OnAdStarted();','//this.OnAdStarted();');
	tmp = tmp.replace("AdManager.MaybeShowAd(()=>Game.OnStartGamePressed());", "Game.OnStartGamePressed();");
	tmp = tmp.replace('PokiSDK.rewardedBreak().then((success) => poki.OnRewardedAdCompleted(success));','poki.OnRewardedAdCompleted(true)');
	tmp = tmp.replace("'href'", "'length'");
	eval(tmp);
}
